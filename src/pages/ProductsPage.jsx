  ...
  return (
    <PageMotion>
      <section className="border-b border-[#67201f] bg-[#230709]">
        <div className="mx-auto grid max-w-[1200px] items-center gap-8 px-6 py-8 lg:grid-cols-2 lg:px-0">
            ...
            <div className="space-y-1">
              {categories.map(([name, count]) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-xs transition ${category === name ? "bg-[#542018] text-somak-gold2" : "text-white/80 hover:bg-white/5"}`}
                >
                  <span>{name}</span>
          ...
          <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="hidden rounded-xl border border-[#6d2724] p-1 sm:flex">
                  <button
                    ...
                    className={`rounded-lg p-2 ${view === "grid" ? "bg-[#5a1b18] text-somak-gold2" : "text-white/60"}`}
                  >
                    <Grid2X2 size={17} />
                  </button>
                  <button
                    onClick={() => setView("list")}
                    className={`rounded-lg p-2 ${view === "list" ? "bg-[#5a1b18] text-somak-gold2" : "text-white/60"}`}
                  >
                    <List size={17} />
                  </button>
                </div>
            ...
            <div className="mt-7 flex items-center justify-center gap-2">
              {["‹", "۱", "۲", "۳", "›"].map((n, i) => (
                <button
                  key={i}
                  className={`grid h-10 min-w-10 place-items-center rounded-lg border border-[#6d2724] text-sm ${n === "۱" ? "bg-somak-gold text-somak-950" : "text-white/70"}`}
                >
                  {n}
                </button>
              ))}
